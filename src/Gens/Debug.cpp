...
/*	
	...
	if (Debug == DMODE_32_SSH2) sh = &S_SH2;
	else sh = &M_SH2;
	...
	{
		case K_T:
			if ((Debug == DMODE_68K) || (Debug == DMODE_VDP))
			{
				main68k_tripOdometer();
				main68k_exec(1);
			}
			else if (Debug == DMODE_Z80)
			{
				z80_Clear_Odo(&M_Z80);
				z80_Exec(&M_Z80, 1);
			}
			else if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_GFX))
			{
				sub68k_tripOdometer();
				sub68k_exec(1);
			}
			else if ((Debug >= DMODE_32_MSH2) && (Debug <= DMODE_32_VDP))
			{
				SH2_Clear_Odo(sh);
				SH2_Exec(sh, 1);
			}
		...
		case K_H:
			/* generate interrupt type 1 */
			if (Debug == DMODE_68K) main68k_interrupt(4, -1);
			else if (Debug == DMODE_Z80) z80_Interrupt(&M_Z80, 0xFF);
			else if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_GFX)) sub68k_interrupt(5, -1);
			else if ((Debug >= DMODE_32_MSH2) && (Debug <= DMODE_32_VDP)) SH2_Interrupt(sh, 8);
			break;

		case K_J:
			/* generate interrupt type  2 */
			if (Debug == DMODE_68K) main68k_interrupt(6, -1);
			else if (Debug == DMODE_Z80) z80_Interrupt(&M_Z80, 0xFF);
			else if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_GFX)) sub68k_interrupt(4, -1);
			else if ((Debug >= DMODE_32_MSH2) && (Debug <= DMODE_32_VDP)) SH2_Interrupt(sh, 12);
			break;
		...
		case K_C:
			if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_CDC)) SCD.Cur_LBA++;
			VDP_Status ^= 0x8;
			break;
		...
		case K_N:
			/* skip 1 */
			if ((Debug >= DMODE_32_MSH2) && (Debug <= DMODE_32_SSH2))
			{
				sh->PC += 2;
				sh->Status &= 0xFFFFFFF0;
			}
			else if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_CDC))
			{
				sub68k_context.pc += 2;
			}
			else if (Debug == DMODE_68K)
			{
		...
		case K_W:
			if ((Debug >= DMODE_CD_68K) && (Debug <= DMODE_CD_CDC)) Check_CD_Command();
		...
		case K_SPACE:
			...
			if (Debug) 
			{
				...
				{
					/* skip 32X */
					if (Debug > DMODE_CD_GFX) Debug = DMODE_68K;
				}
				else if (_32X_Started)
				{
					/* skip CD */
					if ((Debug > DMODE_VDP) && (Debug < DMODE_32_MSH2)) Debug = DMODE_32_MSH2;
					if (Debug > DMODE_32_VDP) Debug = 1;
				}
				else
				{
					/* Genny only, skip 32X & CD */
					if (Debug > DMODE_VDP) Debug = 1;
				}
...
unsigned short Next_Word(void)
{
	...
	if (Debug == DMODE_68K) val = M68K_RW(Current_PC);
	else if (Debug >= DMODE_Z80) val = S68K_RW(Current_PC);
...
void Update_Debug_Screen(void)
{
	...
	if (Debug & DMODE_PREVIEW) Do_VDP_Only();
	else switch(Debug)
	{
		default:
		case DMODE_68K:		// Main 68000
			Refresh_M68k_Mem();
			Refresh_M68k_Inst();
			Refresh_M68k_State();
			Refresh_VDP_State();
		...
		case DMODE_VDP:		// Genesis VDP
			Refresh_VDP_Pattern();
			Refresh_VDP_Palette();
			break;

		case DMODE_CD_68K:		// Sub 68000 Reg
			Refresh_S68k_Mem();
			Refresh_S68k_Inst();
			Refresh_S68k_State();
		...
		case DMODE_CD_CDC:		// Sub 68000 CDC
			Refresh_S68k_Mem();
			Refresh_S68k_Inst();
			Refresh_S68k_State();
			Refresh_CDC_State();
		...
		case DMODE_CD_GFX:		// Vector chip pattern
			Refresh_Word_Ram_Pattern();
			break;

		case DMODE_32_MSH2:		// Main SH2
			Refresh_SH2_Mem();
			Refresh_SH2_Inst(0);
			Refresh_SH2_State(0);
			Refresh_32X_State();
