...
int ASPI_Mechanism_State(int async, int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
	...
	se.CDBByte[8]   = 0x01;           /* buffer lenght */
...
int ASPI_Read_CD_LBA(int adr, int lenght, unsigned char sector, unsigned char flag, unsigned char sub_chan, int async, int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
	...
	s->CDBByte[6]      = (lenght >> 16) & 0xFF;
	s->CDBByte[7]      = (lenght >> 8) & 0xFF;
	s->CDBByte[8]      = lenght & 0xFF;
...
int ASPI_Total_Lenght_CDD_c23_COMP(SRB_ExecSCSICmd *s)
{
	ASPI_Command_Running = 0;
