...
void Patch_Codes(void)
{
	...
	for(i = 0; i < 256; i ++)
	{
		if ((Liste_GG[i].code[0] != 0) && (Liste_GG[i].addr != 0xFFFFFFFF) && (Liste_GG[i].active))
		{
			...
			else M68K_WW(Liste_GG[i].addr, Liste_GG[i].data);
		}
	}
...
int Load_Patch_File(void)
{
	...
	enum etat_sec {DEB_LIGNE, CODE, BLANC, COMMENT, ERR} etat = DEB_LIGNE;
	...
	Lenght = GetFileSize(Patch_File, &Bytes_Read); 
	...
	if (ReadFile(Patch_File, Patch_String, Lenght, &Bytes_Read, NULL))
	{
		i = 0;
		Ind_GG = 0;
			...
			switch(etat)
			{
				...
				case CODE:
					switch(c)
					{
						...
						case ' ':
							Code[i_code] = 0;
							if (check_code(Code, Ind_GG)) etat = BLANC;
							else etat = ERR;
							break;
