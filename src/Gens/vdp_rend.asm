...
;; GENS_KMOD
;; if undef GENS_KMOD, their def val are 0xFF so...no pb! :)
	ALIGN32
...
%ifdef GENS_KMOD		
		call _SpySpr							; si on arrive ici, c que +80sprites
%endif
...
%%Cell_VScroll
	test byte [ActiveLayer], 0x04
	jz near %%NoCell_VScrollB
	RENDER_LINE_SCROLL_B     %1, 1, %2

%%NoCell_VScrollB
	test byte [ActiveLayer], 0x08
	jz near %%Scroll_OK
...
;		jmp .VDP_OK
	...
	.HS_NInterlace
%ifdef GENS_KMOD	
			; no highlight/shadow if one of the layer is OFF
			test byte [ActiveLayer], 0x04
			jz near .NHS_NInterlace
			test byte [ActiveLayer], 0x08
			jz near .NHS_NInterlace
%endif
...
;; GENS KMOD
		test byte [ActiveLayer], 0x10
		jz	near .32X_Draw_M00
;;
